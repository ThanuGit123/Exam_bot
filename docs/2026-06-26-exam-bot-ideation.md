...
Each idea uses the template below. Status legend:
`💡 captured` → `🔍 exploring` → `✅ accepted` (ready for a plan) → `🅿️ parked` → `❌ dropped`
...
| **🗂️ Data & privacy** | view/clear "what Adept remembers" (running summaries) · export my chats · delete history / account · Terms & Privacy | no |
...
- **Decision:** _exploring — focused spec above; confirm v1 pref set + onboarding in/out before promoting to a plan._
...
- **Decision:** _exploring — recommend building **Step 1** first (lowest risk, no RAG); promote to a plan on approval._
...
- **What goes on the Dashboard:**
  ...
  - **🗓️ Exam countdown:** days to target exam date + syllabus coverage %
  ...
  - **🗺️ Knowledge map:** prerequisite gaps / "fix this first" (from Idea 8) *(needs grading)*
- **Fit with current architecture:**
...
- **Decision:** _exploring — agree the v1 widget set + shell before building._
...
- **Decision:** _exploring — high value, mostly prompt + reflect._
...
- **Decision:** _exploring — killer feature; build on the grading node._

### Idea 7 — Personal mistake / error bank   `🔁 folded into Idea 13`
...
- **Decision:** _implement as part of Idea 13's `learning_records`._
...
- **Decision:** _exploring — powerful but content-heavy; phase after grading._
...
- **Decision:** _exploring — cheap, high-impact; light version is prompt-only._
...
- **Decision:** _exploring — clear value (esp. UPSC current affairs); slots into Phase 2 tools._
...
- **Decision:** _exploring — high value, low effort, frontend-only; strong early win._
...
- **Decision:** _parked — strategic; revisit once the single-student experience is strong._
...
- **Decision:** _exploring — backbone of real adaptation; supersedes Idea 7._
...
- **Decision:** _exploring — cheap, high-leverage; fold into Idea 1 onboarding._
...
- **Decision:** _exploring — the "why it works" engine; interleaving is a quick win._
...
- **Decision:** _exploring — strong revisitable artifact; after RAG + renderer._
...
- **Decision:** _exploring — high revision value; builds on the learner model._
...
- **Decision:** _exploring — the trust backbone for RAG; pairs with the content pipeline._
...
- **Decision:** _exploring — core to feeling like a real, engaging tutor; flags land in Phase 1._
...
_(quick captures that need more thought)_

---

## Promoted to plans

_(ideas accepted → linked to their implementation plan under `docs/superpowers/plans/`)_
